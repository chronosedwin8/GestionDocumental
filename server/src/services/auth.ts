...
export async function validatePasswordStrength(password: string): Promise<void> {
  ...
  if (password.length < policy.min_length) problems.push(`debe tener al menos ${policy.min_length} caracteres`);
  if (policy.require_uppercase && !/[A-ZÁÉÍÓÚÑ]/.test(password)) problems.push('debe incluir una letra mayúscula');
  if (policy.require_number && !/\d/.test(password)) problems.push('debe incluir un número');
...
export async function login(
  ...
  const user = mapAuthUser({ ...row, last_login_at: new Date().toISOString() });
